<template>
  <div>
    <article v-for="post in posts" :key="post.id">
      ...
      <div class="information">By {{ findUser(post.userId).login}}</div>
      ...
      <div class="footer">
        ...
        <div class="right">
          ...
          <a href="#">{{ mapOfComments.get(post.id) }}</a>
        </div>
      </div>
    </article>
  </div>
...
export default {
    name: "Index",
    props: ["posts", "users", "mapOfComments"],
    methods: {
      findUser: function(userId) {
        return Object.values(this.users).filter(u => u.id === userId)[0];
      }
    }
}
</script>
