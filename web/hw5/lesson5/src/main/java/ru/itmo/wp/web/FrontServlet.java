    ...
    private void process(Route route, HttpServletRequest request, HttpServletResponse response)
        ...
        for (int i = 0; i < method.getParameterCount(); i++) {
            if (method.getParameterTypes()[i] == Map.class) {
                parameters[i] = view;
            } else if (method.getParameterTypes()[i] == HttpServletRequest.class) {
        ...
        if (request.getSession().getAttribute("lang") != null && !request.getSession().getAttribute("lang").equals("en")) {
            try {
